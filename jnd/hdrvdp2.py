...
def check_if_values_plausible(img, metric_par):
    ...
    if (~metric_par.disable_lowvals_warning):
        ...
        if (np.max(img) <= 1):
            ...
            #### added by Claude:
def compute_sensitivity_map(image, color_encoding, pixels_per_degree, options={}):
    """
    Returns a per-pixel map of aggregate perceptual sensitivity.
    """
    metric_par = Metric_par(options)
    metric_par.pix_per_deg = pixels_per_degree
    metric_par.color_encoding = color_encoding.lower()
    ...
    # Color encoding conversion (same as hdrvdp2)
    if metric_par.color_encoding == 'srgb-display':
        image = display_model_srgb(image)
    elif metric_par.color_encoding == 'luma-display':
        image = display_model(image, 2.2, 99, 1)
    elif metric_par.color_encoding == 'xyz':
        image = xyz2rgb(image)

    # Surround luminance setup — this is what's causing your error
    if metric_par.surround_l == -1:
        metric_par.surround_l = geomean(
            np.reshape(image, [image.shape[0] * image.shape[1], image.shape[2]]))
    elif hasattr(metric_par, 'surround_l'):
        metric_par.surround_l = np.repeat(metric_par.surround_l, img_channels)

    # Spectral emission setup
    if img_channels == 3:
    ...
    [B, L_adapt, band_freq, bb_padvalue] = hdrvdp_visual_pathway(image, 'reference', metric_par, -1)
    
    [B, L_adapt, band_freq, bb_padvalue] = hdrvdp_visual_pathway(image, 'reference', metric_par, -1)
    
    # Precompute CSF lookup
    csf_la = np.logspace(-5, 5, 256)
    csf_log_la = np.log10(csf_la)
    b_count = B['sz'].shape[0]
    ...
    # Accumulate sensitivity across bands
    sensitivity_map = np.zeros_like(L_adapt)
    for b in range(b_count - 1):  # skip base band
        log_La_rs = clamp(imresize(log_La, output_shape=get_band_size(B, b, 0)),
                          csf_log_la[0], csf_log_la[-1])
        CSF_b = interpolate.interp1d(csf_log_la, CSF[:, b])(log_La_rs)
        # Resize back to full resolution and accumulate
        CSF_b_full = imresize(CSF_b, output_shape=L_adapt.shape)
        sensitivity_map += CSF_b_full
