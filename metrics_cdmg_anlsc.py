...
# filename = 'doa_results_DonutModel.pkl'
# path = rf"/home-local/mpintore/recovery_2109/adv_docVQA/resultsFORWARD/Exp4_cluster130_doa/doa-4-bottom_right_corner-DonutModel/{filename}"

# filename = 'doa_results_Pix2StructModel.pkl'
# path = rf"/home-local/mpintore/recovery_2109/adv_docVQA/resultsFORWARD/Exp4/doa-3-bottom_right_corner-Pix2StructModel/{filename}"

filename = 'results_DonutModel.pkl'
path = rf"/home-local/mpintore/recovery_2109/adv_docVQA/resultsACCUMULATION/Exp3_cluster130/4-bottom_right_corner-DonutModel/{filename}"
...
# doa path
# filename = 'results_Pix2StructModel.pkl'
# path = rf"/home-local/mpintore/recovery_2109/adv_docVQA/resultsACCUMULATION/Exp2/5-bottom_right_corner-Pix2StructModel/{filename}"

# model = 'Pix2StructModel'
# filename = f'results_{model}.pkl'
# path = rf"/home-local/mpintore/recovery_2109/adv_docVQA/resultsFORWARD/Exp4_cluster130_multi/4-bottom_right_corner-{model}/{filename}"
...
def report(results):
    ...
    for item_key, value in results.items():
        ...
        for i in range(batch_size): # for each element in the batch
            ...
            #     all_samples_in_batch_match = False
            if DOA and y_pred_str in gt_list:
                all_samples_in_batch_match = False

            pred_adv_norm = evaluator._preprocess_str(y_pred_adv_str)
            y_pred_norm = evaluator._preprocess_str(y_pred_str)
            # if not DOA:
            ...
            anls_c_score = evaluator._calculate_anls([y_pred_norm], pred_adv_norm, answer_type="string")
            total_anls_c.append(anls_c_score)
    ...
    final_cdmg = (sum(cdmg) / len(cdmg)) * 100 if cdmg else 0.0
    ...
    if total_samples_processed == 0:
        ...
        final_anls_c = (sum(total_anls_c) / len(total_anls_c)) * 100 if total_anls_c else 0.0
    ...
    print(f"# CDMG (Sample-level): {final_cdmg:.2f}%")
    print(f"# ANLS_C (Sample-level): {final_anls_c:.2f}%")
    print('******************************')
